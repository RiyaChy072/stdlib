...
void extract_words(double x, int32_t * high, int32_t * low) {
	double_parts t;
	t.d = x;
	*high = t.parts.high;
	*low = t.parts.low;
...
void set_high_word(double *x, int32_t high) {
	double_parts t;
	t.d = *x ;
	t.parts.high = high;
	*x = t.d;
...
double stdlib_base_atanh( const double x ) {
	...
	//Extract words from x
    extract_words(x, &hx, &lx) ;

    if((hx & 0x7fffffff) > 0x3ff00000) { // |x|>1 or x is NaN
		return 0.0 / 0.0 ; //NaN
	}

	if((hx & 0x7fffffff) == 0x3ff00000) { // |x| == 1
	    return (hx > 0) ? STDLIB_CONSTANT_FLOAT64_PINF : STDLIB_CONSTANT_FLOAT64_NINF;
	}
